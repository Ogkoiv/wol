...
#sender
def sender(finalarray,broadcast_addr, broadcast_port):
    udp_socket = socket.socket(socket.AF_INET, socket.SOCK_DGRAM)
    udp_socket.setsockopt(socket.SOL_SOCKET, socket.SO_BROADCAST, 1)
    print(bytes(finalarray))
    udp_socket.send(bytes(finalarray), (broadcast_addr, broadcast_port))
    udp_socket.close()
...
if arg=="peli": 
    ...
    sender(finalarray,broadcast_addr,broadcast_port)
